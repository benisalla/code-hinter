...
def delete_stud_exe(assoc_id):
    associations = [assoc for assoc in load_stud_exe() if assoc["id"] != assoc_id]
    save_stud_exe(associations)
    return True

def get_associations_by_student_id(student_id):
    return [assoc for assoc in load_stud_exe() if assoc.get("student_id") == student_id]

def get_associations_by_exercise_id(exercise_id):
    return [assoc for assoc in load_stud_exe() if assoc.get("exercise_id") == exercise_id]

def get_associations_by_prof_id(prof_id):
    return [assoc for assoc in load_stud_exe() if assoc.get("prof_id") == prof_id]

def get_submitted_students_by_exercise_id(exercise_id):
    associations = load_stud_exe()
    result = []
    for assoc in associations:
        if assoc.get("exercise_id") == exercise_id and assoc.get("status") == "done":
...
def get_not_done_exercises_by_student_id(student_id):
    ...
    result = []
    for assoc in associations:
        if assoc.get("student_id") == student_id and assoc.get("status") != "done":
